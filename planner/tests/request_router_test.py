...
import os
import pytest
from planner.orchestrator.request_router import route, IncompleteRequestError
...
def test_route_extracts_complete_request():
    result = route({"user_request": "Plan a 4-day trip to Lisbon, Portugal for $900"})
...
def test_route_raises_on_missing_num_days():
    with pytest.raises(IncompleteRequestError) as exc_info:
        route({"user_request": "I want to go to Lisbon"})
...
def test_route_budget_is_optional():
    ...
    result = route({"user_request": "Plan a 3-day trip to Tokyo, single city"})
